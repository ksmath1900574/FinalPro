...
public interface BiticBoardRepository extends JpaRepository<BiticBoardEntity, Long>{
    ...
    // 제목으로 필터링
    Page<BiticBoardEntity> findByTitleContaining(String title, Pageable pageable);

    // 태그와 제목으로 동시에 필터링
    Page<BiticBoardEntity> findByTagAndTitleContaining(String tag, String title, Pageable pageable);
